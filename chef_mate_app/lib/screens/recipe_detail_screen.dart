...
class RecipeDetailScreen extends StatefulWidget {
  ...
  const RecipeDetailScreen({super.key, required this.recipeId, this.missingIngredients = const []});
...
class _RecipeDetailScreenState extends State<RecipeDetailScreen> {
  ...
  String _normalizeIngredient(String ingredient) {
    ...
    cleaned = cleaned.replaceFirst(RegExp(r'^(tbsp|tbs|tsp|tspn|g|ml|kg|l|oz|lb|cup|cups|pinch|dash|cloves?|slices?|pieces?|cans?|packages?|bunch|handful|sprigs?|leaves?|sticks?)\s+'), '');
  ...
  Widget _buildIngredientsSection(List<String> ingredients) {
    ...
    final showStatus = widget.missingIngredients.isNotEmpty;
    final missingSet = widget.missingIngredients.toSet();
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          ...ingredients.map((ing) {
            final normalized = _normalizeIngredient(ing);
            final isMissing = missingSet.contains(normalized);
            return Padding(
              padding: const EdgeInsets.only(bottom: 6),
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  if (showStatus)
                    Padding(
                      ...
                      child: Icon(
                        isMissing ? Icons.close : Icons.check_circle,
                        size: 16,
                        color: isMissing ? Colors.red : AppTheme.accentGreen,
                      ),
                    )
                  else
                    const Padding(
                  ...
                  Expanded(
                    child: Text(
                      ...
                      style: TextStyle(
                        fontSize: 14,
                        color: showStatus && !isMissing ? AppTheme.accentGreen : null,
                      ),
                    ),
                  ),
                ],
          ...
          Padding(
            ...
            child: Card(
              child: Padding(
                ...
                child: Column(
                  children: [
                    RatingInputWidget(rating: _userRating, onRatingUpdate: (v) => setState(() => _userRating = v)),
                    const SizedBox(height: 8),
                    TextField(
