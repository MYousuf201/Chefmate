...
router.get('/', optionalAuth, async (req, res) => {
  try {
    ...
    let query = `
      SELECT r.*, u.email as creator_email, u.name as creator_name, u.username as creator_username, u.id as creator_id
...
router.get('/favorites/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    const [favorites] = await pool.execute(`
      SELECT r.*, u.email as creator_email, u.name as creator_name, u.username as creator_username,
...
router.post('/scan-ingredients', authenticateToken, upload.single('image'), async (req, res) => {
  try {
    ...
    const message = error.message || 'Failed to scan ingredients';
